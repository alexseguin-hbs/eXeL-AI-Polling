...
ok(/function rangeReset\(\)\{ (state\.lastShot=null; )?(state\.pending=null; )?(magLoad\('RESET'\); )?(\/\*[^*]*\*\/ ?)?const up=rangeTraining\(\);/.test(html) && /qualResetPlatesForTable\(\)\{[\s\S]{0,400}q\.up=false/.test(html), 'r.137: RESET stands every target in training; a qualification starts with every plate DOWN');
