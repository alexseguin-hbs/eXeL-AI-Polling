...
const AFTER_FILL = new Set([]);   // keys added after the last fill pass: English until the next pass — listed here, never silent
...
for (const code of codes) {
  ...
  const missing = keys.filter((k) => !AFTER_FILL.has(k) && !String(m[k] ?? '').trim());
  ...
  const same = keys.filter((k) => m[k] && m[k] === en[k].englishDefault && !KEEP.test(en[k].englishDefault));
