...
import { DEFAULT_ENGLISH_TRANSLATIONS as __EN } from "../lib/lexicon-data.ts";

// SoI-2525 UX is now keyed for i18n: its visible strings render via t("key"). These structural
// invariants match the ENGLISH text, so before matching we resolve t("key") back to its English
// default — the guarantee is unchanged (same text, same layout); only the delivery mechanism moved
// to the lexicon (whole-site UX translation, 2026-09-13). A layout regression still fails the gate.
// Resolve ONLY the batch-6 soi2525.* keys back to English (the strings this pass moved into the
// lexicon). Pre-existing t("innovation.*") calls are left intact — some assertions match those keyed
// forms directly.
const __resolveT = (src) => src
  .replace(/=\{\s*t\("(soi2525\.[^"]+)"\)\s*\}/g, (_m, k) => `="${(__EN[k]?.englishDefault ?? k)}"`)
  .replace(/\{\s*t\("(soi2525\.[^"]+)"\)\s*\}/g, (_m, k) => (__EN[k]?.englishDefault ?? k));
...
{
  const F = await import("../lib/innovation-data.ts");
  const src = await (await import("node:fs/promises")).readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
   ...
   producers to the template AND to that differentiation. Every one was written RED first.               */
{
  ...
  const pageSrcCsra = (await (await import("node:fs/promises")).readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT));
...
// passes on a blank deck, which is worse than no gate at all.
{
  ...
  const src = await fsp.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// that proves something is behind that door, and that typing into it does not wipe the rest of the plan.
{
  ...
  const pageSrc = await fsp2.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// `finPlan`; that block wrote `revPlan`. Two money editors stacked on one panel disagree by construction.
{
  ...
  const src = await fsp0.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// value propositions for the same project.
{
  ...
  const src = await fspv.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// could not have held a number anyway.
{
  ...
  const src = await fspc.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// entirely and the grid — which cannot be collapsed — is the whole slide.
{
  ...
  const pageE = await fspE.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
  const p0 = F.DEMO_PROJECTS[0];
...
// The read-out promised figures the input surface would not accept.
{
  ...
  const pageE1 = await fspE1.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// has to be pinned instead. Structural here; measured in a browser by the E2 proof.
{
  ...
  const pageE2 = await fspE2.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// already is when someone wonders "can I change this?".
{
  ...
  const pageE3 = await fspE3.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// re-attach a transform to the body and this goes red; drop `* zoom` from the sheet and this goes red.
{
  ...
  const pageZ = await fspZ.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// are inputting project data, we do not see commercial details, and vice versa."
{
  ...
  const pageB = await fspB.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// and low or Med Commercial. and 4 is other option (low/med commercial risk and tech risk)."
{
  const F = await import("../lib/innovation-data.ts");
  const fspC = await import("node:fs/promises");
  const pageC = await fspC.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// `valuePropOf`, `briefOf`) that never name the field. Mutation is the only honest test.
{
  ...
  const pageR = await fspR.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
{
  const fspz = await import("node:fs/promises");
  const src = await fspz.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// and never looked at a bar. Labels are not bars. That is the same class as the stale slide-shots locator.
{
  ...
  const p3 = await fspP3.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// position is algebraically identical to capture, and value-for-money is not a price axis.
{
  ...
  const pgD3 = codeOnly(await fspD3.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT));
...
// converting both wrapper sites at once is safe rather than risky.
{
  ...
  const d8 = await fspD8.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// and was proven RED (12 failures, ratio 1.00 where 3.00 was required) before the fix landed.
{
  ...
  ok(/from "@\/lib\/use-viewport"/.test(await fspV.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT)),
...
// Probe error #17. The probe now keys on structural path and refuses to report unless ≥80% of paths match.
{
  ...
  const p1 = await fspP1.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// screenshot and are very different in a screen reader.
{
  ...
  const srcW2 = await fspW2.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// the DECLARATION (colgroup + table-fixed), never about the two tables looking similar.
{
  ...
  const srcW3 = await fspW3.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// Asserted as ORDER OF FIRST APPEARANCE in the rendered tree, so re-adding a panel above it fails.
{
  ...
  const srcW6 = await fspW6.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// ── W-12 · THE RACK HEADER FREEZES · H5 · THE FOOTER REACHES BOTH EDGES ─────────────────
{
  ...
  const srcW12 = await fspW12.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// since A1; only the render did not. Proven below by EXECUTING it over the real portfolio, not by reading.
{
  ...
  const srcW8 = await fspW8.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// as before … or whatever we used to have for Technical and Commercial Confidence."
{
  ...
  const srcW13 = await fspW13.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// Prop numbers." Their screenshot showed `VALUE CAPTURE % 3` after typing 33 — a DROPPED KEYSTROKE.
{
  ...
  const srcW14 = await fspW14.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// ── W-7 · PRICE PERFORMANCE: COMPETITION — editable markers, and the disk-not-pin convention ─────────
{
  ...
  const srcW7 = await fspW7.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// ── W-7b · THE SAVE ICON IS A DISK, REPO-WIDE. "Never use a pin." ────────────────────────────────────
{
  ...
  const pageSave = await fspSave.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// `15 funded` as a whisper, so the headline was the number nobody acts on.
{
  ...
  const srcW15 = await fspW15.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// show Alpha Group in front of Title. If Alpha Group, show Alpha Code in front of Title."
{
  ...
  const codeW17 = (await fspW17.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT))
...
// same slide, in a COLLAPSED panel.
{
  ...
  const codeW18 = (await fspW18.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT))
    .replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");
...
// ── W-19 + W-20 · THE STACKED GOLDEN PRICE BAR, AND THE 3D BEVEL ─────────────────────────────────────
{
  ...
  const raw = await fspW19.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// the minus was contradicting the word printed directly beneath it.
{
  ...
  const raw21 = await fspW21.readFile("app/SoI-2525/page.tsx", "utf8").then(__resolveT);
...
// outcome the number has not earned. Locked as ORDER + NAME + TONE, because all three were the ask.
{
  ...
  const srcX9 = await fspX9.readFile(new URL("../app/SoI-2525/page.tsx", import.meta.url), "utf8").then(__resolveT);
...
// nothing the moment ONE ancestor sizes to content. Measured cost of the last break: S8 clipped 258px.
{
  ...
  const srcX1d = await fspX1d.readFile(new URL("../app/SoI-2525/page.tsx", import.meta.url), "utf8").then(__resolveT);
...
// FLIR twelve the lib already documents, and removing them deleted a LIVE BUG rather than relabelling one.
{
  ...
  const srcX4 = await fspX4.readFile(new URL("../app/SoI-2525/page.tsx", import.meta.url), "utf8").then(__resolveT);
...
// ── X-1c · THREE BANDS · X-5a · CAPTURE % IS AN INPUT ───────────────────────────────────────────────
{
  ...
  const srcX5 = await fspX5.readFile(new URL("../app/SoI-2525/page.tsx", import.meta.url), "utf8").then(__resolveT);
