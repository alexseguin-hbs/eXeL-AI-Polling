...
ok(new RegExp(`revision:'0\\.${REV}'`).test(src) && !/r0\.129\b/.test(src) && !/r0\.123/.test(src), `r.${REV} declares itself and carries no stale bookkeeping`);
// r.129 — still present
...
ok(!/Math\.atan2\(dx,-dz\)/.test(src) && (src.match(/yawTo\(dx,dz\)/g) || []).length >= 5, 'every aim goes through yawTo (no atan2(dx,−dz) survivor of the old basis)');
...
ok(/function plateHit\(q\)/.test(src) && /const hit=!!state\.simDirect \|\| \(ph\?ph\.hit:off<32\);/.test(src) && /const PIP_FLOOR_PX=6;/.test(src), 'a silhouette is hit when the pip is inside its projected outline (scale-true), with a declared 6 px floor');
...
ok(/function drawPlates\(segs\)/.test(src) && /if\(chNum\(\)===0\)\{ drawPlates\(segs\); RANGE_WIRE\.forEach/.test(src), 'on the range the exposed silhouettes take the segment budget before the wire (r.128/r.129 dropped every target at MoT 1.1)');
for (const row of ['PROJ_FWD_AGREES', 'FWD_IS_WHAT_YOU_SEE_', 'RANGE_SEES_PLATES', 'ALTC_LAYOUT', 'RANGE_POP_SCHEDULE', 'RANGE_LOCK_PLATE', 'RANGE_HIT_50', 'RANGE_HIT_300', 'RANGE_MISS_300_OFF20', 'RANGE_HIT_50_OFF10', 'MODE_RESET_RETURNS', 'MODE_DOWN_STAYS', 'MODE_QUAL_TIMED']) ok(src.includes(`push('${row}`), `QA row ${row} present`);
...
ok(/data-drone-play\b/.test(ux) && /\/drone-2525\/play\.html/.test(ux), 'the /drone-2525 shell offers PLAY → the served deck');
