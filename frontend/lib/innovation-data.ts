...
export const PROJECT_BRIEF: Record<string, ProjectBrief> = {
  ...
  "PRJ-34": {
    needs: ["One governed environment for strategic judgment and coordinated leadership action across reputation, operations, legal, regulatory, financial, safety and stakeholder domains", "Faster time to coherent, approved leadership action with a traceable record of what was known, decided, why it changed, who owned the action and what happened next"],
    ...
    solution: ["One platform: the governed leadership record (decision rights · approval · ownership · acknowledgement · evidence · current plan · replay) plus, by qualification, every aspect of the benchmark's critical-event job — Everbridge 360 AI / Bridge: alert, workflow, continuity, risk feed", "Absorb by the R-CORE ladder: every benchmark function reachable from inside the platform in 2027, native function by function in 2030 after each beats the benchmark on its pre-registered clock by the one-ninth gate, life-safety alerting last and certified; Rave and PagerDuty facts flow in as inbound feeds", "Pro Team → Pro Enterprise commercial spine only"],
