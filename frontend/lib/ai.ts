...
export type AiProvider = "auto" | "openai" | "gemini" | "grok";
export interface AiConfigured { openai: boolean; gemini: boolean; grok: boolean }
...
export async function aiStatus(): Promise<AiConfigured> {
  try { const d = await jsonOf(await fetch("/api/ai")); const c = (d.configured ?? {}) as Partial<AiConfigured>; return { openai: !!c.openai, gemini: !!c.gemini, grok: !!c.grok }; } catch { return { openai: false, gemini: false, grok: false }; }
}
export const anyAi = (c: AiConfigured) => c.openai || c.gemini || c.grok;
export async function aiPlace(image: string, signer: string, provider: AiProvider = "auto", hint = ""): Promise<{ provider: string; result: AiPlace | null } | null> {
  const d = await jsonOf(await fetch("/api/ai", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ task: "place", image, signer, hint, provider }) }));
...
export async function aiDraft(prompt: string, lang: string, provider: AiProvider = "auto"): Promise<{ provider: string; result: AiDraft } | null> {
  const d = await jsonOf(await fetch("/api/ai", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ task: "draft", prompt, lang, provider }) }));
  if (d.configured === false) return null; if (d.error) throw new Error(String(d.error));
