...
export async function aiStatus(): Promise<AiConfigured> {
  try { const d = await jsonOf(await fetch("/api/ai")); const c = (d.configured ?? {}) as Partial<AiConfigured>; return { openai: !!c.openai, gemini: !!c.gemini, grok: !!c.grok, claude: !!c.claude }; } catch { return { openai: false, gemini: false, grok: false, claude: false }; }
}
...
export async function aiDraft(prompt: string, lang: string, provider: AiProvider = "auto", signers: { role: string; name: string }[] = []): Promise<{ provider: string; result: AiDraft } | null> {
  const d = await jsonOf(await fetch("/api/ai", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ task: "draft", prompt, lang, provider, signers }) }));
  if (d.configured === false) return null; if (d.error) throw new Error(String(d.error));
