...
export const DRONE_DOMAIN = {
 ...
 "revisions": [
  ...
  },
  {
   ...
   "date": "2026-09-23",
   "kind": "release",
   "why": "Operator deck HEAD r.142: reload by hand on QUAL, the same order as the actual test. The engagement program names its silhouettes (left / centre / right) and is the same on every lane, every time — the r.137 per-lane seed retired by operator decision so a shooter can memorise the order; ten targets a phase = one magazine; the tower's rest says PRESS RELOAD and loads nothing; an empty magazine refuses FIRE; four magazines as issued (a fifth refused NO_MAGAZINE). QA 143 rows, 142/143 in each orientation.",
   "commit": "662f8e3"
  },
  {
   "revision": "0.028",
 ...
 "crs": [
  ...
  {
   "id": "DRN-10.09",
   "title": "CH1–CH5 and DIFF 1–5, from the deck",
   ...
   "phase": "pilot",
   "mode": "Manual",
   "metric": "25 CH×DIFF cells equal to r.050's function; harder is never more time; CH5 self-approval refused and on the record",
   "verify": "frontend/tests/drone-challenge.test.mjs",
   "dtm": "a player feels the rung, and CH5 needs two people",
   "stretch": "moving targets from the swarm at CH3+",
   "status": "implemented"
  },
  {
   "id": "DRN-06.02",
   "title": "The AsM Cup as a regression fixture",
   ...
   "phase": "pilot",
   "mode": "Manual",
   "metric": "two hashes pinned; four baseline sentences recomputed; roster = the twelve; sidecar path ≠ fixture path; the module imports no filesystem",
   "verify": "frontend/tests/asm-cup.test.mjs",
   "dtm": "a later build is judged against his published result",
   "stretch": "the contest replays the Cup at seed 2525 and compares his six",
   "status": "implemented"
  },
  {
   "id": "DRN-09.04",
   "title": "The second person, over the link",
   ...
   "phase": "pilot",
   "mode": "Manual",
   "metric": "approve authored by either seat, needs door + slot; duplicate lands once; own echo dropped; holder refuses self",
   "verify": "frontend/tests/drone-link.test.mjs",
