...
.frame{position:relative;overflow:hidden;flex:1;margin:14px;border:3px solid var(--clr,#ef4444);border-radius:10px;display:flex;flex-direction:column}
...
.codexTL{position:absolute;top:5px;left:5px;image-rendering:pixelated;opacity:.92}
.codexBR{position:absolute;bottom:5px;right:5px;image-rendering:pixelated;opacity:.92}
...
<div class="wrap">
  <div class="frame" id="frame">
    <!-- Light Codex double helix: forward upper-left, reverse bottom-right (4x4 px) -->
    <canvas class="codexTL" id="cxTL"></canvas>
    <canvas class="codexBR" id="cxBR"></canvas>
    <!-- COVER (vague; no title). Clearance shown only as a growing Seed of Life. -->
    <div class="cover" id="cover">
      <div class="seal" id="seal"></div>
      <div class="wonder">Some doors open only for the one who already carries the key.</div>
      <div class="hint">Read freely — it speaks to you alone</div>
      <div>
...
(function(){
  ...
  frame.style.setProperty('--clr',PKG.color);
  ...
  (function(){var r=26,pos=[[0,0]];for(var i=0;i<6;i++){var a=i*Math.PI/3-Math.PI/2;pos.push([r*Math.cos(a),r*Math.sin(a)]);}
    ...
    document.getElementById('seal').innerHTML='<svg width="'+w.toFixed(0)+'" height="'+h.toFixed(0)+'" viewBox="0 0 '+w.toFixed(1)+' '+h.toFixed(1)+'">'+g+'</svg>';})();
  // Light Codex double helix — 4x4 px blocks, forward upper-left, reverse bottom-right.
  (function(){var CM={B:[0,0,0],R:[255,0,0],Y:[255,255,0],G:[0,255,0],C:[0,255,255],V:[255,0,255],W:[255,255,255]},BLK=4;
  ...
  document.getElementById('meta').textContent='LIGHT CODEX · '+PKG.codexDate+' · '+PKG.cstTime+' · '+PKG.sender;
  ...
  function drawLeft(){var pos=seedPositions(),g='';for(var i=0;i<pos.length;i++){var on=i<LVL;
  ...
  function render(){var s=DATA.sections[sec];document.getElementById('rtag').innerHTML='<span>'+s.tag+'</span> <span class="title">· '+s.title+'</span>';document.getElementById('rcontent').textContent=s.content[tier]||s.content[333];document.getElementById('pos').textContent=(sec+1)+' / '+DATA.sections.length;drawTiers();drawLeft();}
  document.getElementById('unlock').onclick=async function(){var c=document.getElementById('code').value.trim();var e=document.getElementById('err');
    ...
    try{DATA=await decrypt(c);COL=DATA.color;LVL=DATA.clearance;var bd=document.getElementById('badge');bd.textContent='CLEARANCE '+DATA.clearance+' · '+DATA.name;bd.style.color=COL;bd.style.borderColor=COL;
      cover.style.display='none';reader.style.display='flex';render();}catch(err){e.textContent='Incorrect code.';}};
