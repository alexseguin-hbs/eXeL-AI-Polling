 ...
 * "Saved Designs" library, the way Mission-Planning saves missions. Save the current design under a name (persisted),
 * re-load any saved design, download it as a portable .arch2525 file, or upload one from disk. All state assembly +
 * seating is owned by command-ux1 (which has the full layer snapshot + homeType + room layout); this is the UI.
 */
...
export function ArchitectProjects({ buildFile, onLoad }: {
  ...
  return (
    <div data-arch-projects className="flex flex-col gap-2 text-[11px]" style={{ color: C.text }}>
      ...
      <div className="text-[8px] font-semibold uppercase tracking-wider" style={{ color: C.dim }}>Saved designs · {lib.length}</div>
      {lib.length === 0 ? (
        ...
        <div className="flex flex-col gap-1">
          {lib.map((e) => (
            <div key={e.name} data-arch-proj-row={e.name} className="flex items-center gap-1 rounded border px-2 py-1" style={{ borderColor: C.border, background: C.panel }}>
