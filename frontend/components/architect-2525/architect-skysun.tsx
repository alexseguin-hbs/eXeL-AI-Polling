...
export function ArchitectSkySun() {
  ...
  const monthDay = new Date(2025, 0, doy).toLocaleDateString(undefined, { month: "short", day: "numeric" });
    ...
    <div className="grid gap-3 lg:grid-cols-[1fr_260px]">
      ...
      <div className="space-y-2 rounded-lg border p-3 text-[11px]" style={{ borderColor: C.border, background: C.panel }}>
        ...
        <div className="border-t pt-1 text-[10px]" style={{ borderColor: C.border }}>
          ...
          <div className="mt-1 text-[9px]" style={{ color: C.dim }}>Polaris altitude ≈ {polaris.el.toFixed(0)}° (your latitude). Deterministic → replayable.</div>
        </div>
      </div>
    </div>
  );
