...
export function Handoff({ link, sender, title, nextName, nextContact }: { link: string; sender: string; title: string; nextName: string; nextContact: string }) {
  ...
  return (
    <div className="rounded-lg border border-primary/40 bg-primary/5 p-4">
      ...
      <div className={`mt-3 flex flex-wrap gap-2${mailFirst ? " flex-row-reverse justify-end" : ""}`}>
        <a href={sms} className={mailFirst ? SECONDARY : PRIMARY}><span aria-hidden="true">💬 </span>{t("soi.sign.handoff.sms")}</a>
        {kind === "email"
