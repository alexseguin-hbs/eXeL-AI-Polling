...
// darken a #RRGGBB hex (multiply RGB by f) — the cone-of-silence is a DARKER shade of the shell colour.
function darkerHex(hex: string, f = 0.62): string {
...
// is sector k (of `sides`) RADIATING per the mil sector-mask? (mask tiles 0→6400; absent = all on)
function sectorOn(k: number, sides: number, mask: CovSeg[] | undefined): boolean {
  if (!mask || mask.length === 0) return true;
  const midMil = ((k + 0.5) / sides) * 6400;
  for (const seg of mask) if (midMil < seg.toMil) return seg.on;
  return false; // past the last boundary → blank
}
// 3-LEVEL (trinity) 13-sided radar coverage SOLID → ONE svg's worth of PROJECTED screen-space faces.
// 4 faces per sector (RADAR-BASE floor, BOTTOM→MID wall, MID→TOP wall, TOP→RADAR cone-of-silence) × sides.
// Orthographic tilt: p=pitch (0 top-down → 90 horizon), bearing rotates azimuth; cx,cy,L in px. Every vertex
// is exactly L (=75 km) from the radar → constant range on every spoke. Blanked sectors emit NO faces (cutouts).
type FaceKind = "base" | "wall1" | "wall2" | "cos";
type SolidFace = { pts: string; depth: number; kind: FaceKind };
...
function radarSolidFaces(fx: number, fy: number, pw: number, ph: number, L: number, altPx: number,
  ...
  for (let k = 0; k < sides; k++) {
    ...
    push([A, G0, G1], "base");        // 1 RADAR BASE (ground floor slice)
...
function GlobeView({ data, center, activeKey, onSelect, onDrill, onEnterAo, onFrameCell, coordFmt, showZones, hiddenKeys }: {
  ...
  // Selected grid cell = the cell at SCREEN CENTRE, not the camera nadir. The fixed 3D tilt makes the
  // nadir (LAT0,LON0) project to (CX, CY + R·sin tilt) — well below centre — so the screen-centre point
  // sits `tilt` radians NORTH along the centre meridian. Select THAT cell so the label/strip/readout
        ...
        <svg ref={flatSvg} viewBox={`${flat.x} ${flat.y} ${flat.w} ${flat.h}`} preserveAspectRatio="xMidYMid slice"
          ...
          {[...placed, ...placedSupport].slice(0, 2000).map((u, i) => { /* cap = worst-case guard; well above any realistic placement count so the world strip matches the (uncapped) main map */
            ...
            return [-W, 0, W].map((off) => (
              <circle key={`am${i}_${off}`} data-assetmark cx={bx + off} cy={by} r="2.2" fill={col} stroke="#0a0e14" strokeWidth="0.5" vectorEffect="non-scaling-stroke" opacity="0.9" style={{ pointerEvents: "none" }} />
            ));
          })}
        </svg>
      )}
      {mode === "flat" && (() => {
        ...
        return (
          <>
            ...
            {/* GRID-OFF readout (MGRS + span). GRID-ON readout is the shared overlay below, so 2D & 3D match. */}
            {!showZones && (
