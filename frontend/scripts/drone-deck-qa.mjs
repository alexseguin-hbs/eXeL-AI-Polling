...
for (const vp of VIEWPORTS) {
  ...
  for (const [id, re] of [['RANGE_HIT_50', /aimed/], ['RANGE_HIT_300', /aimed/], ['RANGE_HIT_50_OFF2', /dead=true/], ['TARGETN_HITS_THE_EXPOSED_PLATE', /lands/], ['FULL_FILLS_THE_STAGE', /whole screen/], ['FULL_BEATS_DESK', /full width/], ['TOP_LINES_NEVER_OVERLAP', vp.name === 'landscape' ? /side by side/ : /stacked/], ['DEVICE_CLASS_BY_SHORT_SIDE', /this window is phone/], /* r.152 */ ['FULLSCREEN_IS_AN_ICON', /off → ⤢\/⤢ .*full → ⤡\/⤡/],
    ...
    /* r.150 — the fire gate reads the picture */ ['FIRE_SAYS_WHERE', /REJECT TARGET_OFF_PICTURE · rounds unchanged \((\d+)→\1\) · the box is still red on C-/], ['FIRE_ON_THE_BOX_STILL_HITS', /FIRE → HIT · rounds (\d+)→(\d+)/], /* r.151 */ ['FIRE_HITS_A_POP', /base \d+ px below the pip, past 32 within \d+\) FIRE spends a round \(\d+→\d+\) where r\.150 refused; on the base \(\d+ px\) FIRE → HIT\/SIM-ACTION \(\d+→\d+\)/], ['FIRE_FROM_MAP_REFUSED', /TARGET_OFF_PICTURE via MAP · rounds unchanged/], ['ONE_ROW_PER_PULL', /2 pulls → 2 rows · 1 APPROVE \(2 MISS rows, approvals (\d+)→\1, box red\)/], ['TAG_NEEDS_RED_BOX', /REJECT TAG_NEEDS_RED_BOX · score unchanged .* no TAG row/], ['PENDING_DIES_WITH_THE_SCENE', /null after RESET \(null\) and after a RELEASE \(null\)/], ['PEER_APPROVE_OF_A_DEAD_PLATE_REFUSED', /HOLD TARGET_DOWN ×2 \(2\), nothing red/], ['APPROVE_LOOKS_AT_THE_MARK', /pan -?\d+\.\d°→-?\d+\.\d° · under the bullseye true/], ['AI_MARK_NEVER_TURNS_THE_SEAT', /the seated head moves 0\.000°/], ['AI_RESUMES_AFTER_RELOAD', /REJECT EMPTY_MAGAZINE then RELOAD then FIRE \(fired\)/], ['FORGED_RED_NEVER_HITS_THE_RING', /REJECT NOT_A_RANGE_TARGET · BULL-1 untouched \((\d+)→\1\) · rounds unchanged/], ['KEY1_REFUSES_THE_GRASS', /key 1 refused · NO TARGET UNDER THE BULLSEYE · nothing marked/], ['TARGET_BUTTON_FOLLOWS_THE_EYE', /the TARGET button marks C-150R(-L\d+)? and the head moves [01]\.\d+° · slots C-150R/], ['RANGE_MISS_300_OFF20', /TARGET_OFF_PICTURE · rounds unchanged/],
