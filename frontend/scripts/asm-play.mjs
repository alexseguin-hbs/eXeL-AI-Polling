...
try {
  if (A.seat === 'solo' || A.seat === 'ai') {
    ...
    for (let i = 0; i < A.shots; i++) {
      if (A.seat === 'ai') { await p.waitForFunction(() => state.desig && state.desig.phase === 'amber', null, { timeout: 20000 }).catch(() => null); await p.evaluate(() => { const u = units[state.unit]; if (u && state.desig && state.desig.ref) aimUnitAt(u, state.desig.ref, -40, 20); }); /* r.148: the AI paints its box and never moves the human's head — the human looks at the box before approving, as a person would */ const d = await hud(p); if (!d.desig) { R.refusals++; R.steps.push({ name: 'AI never marked', ...d }); continue; } await step(p, 'APPROVE', () => p.click('#fAppr')); const f = await step(p, 'FIRE', () => p.click('#fFire')); if (/HIT/.test(f.toast)) R.hits++; else R.misses++; }
