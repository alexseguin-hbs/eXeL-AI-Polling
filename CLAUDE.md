...
- **Iframe framing — allowlist-driven (Full-Embed enabler)** — `SecurityHeadersMiddleware` is default-safe: no `EMBED_ALLOWED_ORIGINS` → `X-Frame-Options: DENY` + CSP `frame-ancestors 'none'` (unchanged). Configure origins → drops X-Frame-Options + emits CSP `frame-ancestors 'self' <origins>` so ONLY those may iframe the app (clickjacking still blocked for everyone else). Remaining embed work: `<exel-polling>` Web Component + postMessage bridge (SDK phase).
- **Web Component `<exel-polling>` — IMPLEMENTED (consumer SDK)** — framework-free custom element at `frontend/public/embed/exel-polling.js` (+ `demo.html`). A consumer includes the script and writes `<exel-polling code="…" host="…">`; it renders the session in a sandboxed iframe, sizes responsively, and exposes a `.send(type, payload)` host→iframe channel.
- **postMessage bridge — IMPLEMENTED (consumer side)** — the Web Component re-dispatches iframe events as origin-checked DOM CustomEvents (`exel:ready|response|themes-ready|ranking-complete|resize|error`); a spoofed frame or non-whitelisted type is rejected (pure core locked by `tests/embed-web-component.test.mjs`, 23 cases, in `test:all`). Remaining: the app's `/session` page emitting the matching `exel-polling` envelopes when embedded (the app-side half — deferred, near Trinity-Redundancy code).
- **API key management — IMPLEMENTED (management + verification; middleware wiring pending)** — per-org `exel_`-prefixed keys (`app/models/api_key.py`), SHA-256-hashed (full key shown once, never stored), org-isolated. Service `app/core/api_key_service.py` (create/list/revoke/`authenticate_api_key` with active+expiry checks, stamps `last_used_at`). Management REST API `app/core/api_key_router.py` (`POST/GET/DELETE /api/v1/api-keys`, JWT-role-gated, listings redact the secret). **Dual-auth dependency wired:** `auth.get_current_principal` (+ pure `resolve_principal`) accepts EITHER `Authorization: Bearer exel_<key>` (→ org-scoped `api_key` principal with the key's scopes) OR an Auth0 JWT — additive, so `get_current_user` stays JWT-only. **Read endpoints adopted:** `GET /usage` + `GET /scoping/projects` + `GET /scoping/projects/{id}/tree` accept an API key via `get_current_principal` (org-scoped; `/usage` still elevated-only for JWTs). Remaining: adopt on write/public-poll endpoints as the SDK matures.
...
- **Usage metering — IMPLEMENTED (foundation)** — append-only per-org `UsageRecord` (`app/models/usage_record.py`: org_id · metric · quantity · cost_tokens ◬ · api_key/session/scope provenance, indexed by org+metric+time). Service `app/core/usage_service.py` (`record_usage` with metric whitelist; `summarize_usage` per-metric + total over an optional window, stable shape). **Billing rollup wired:** `estimate_cost(by_metric)` prices every metric from an authoritative ◬ table (`USAGE_PRICES`) → billable ◬ + USD (`TOKEN_USD_RATE`); `summarize_usage` now carries a `cost` block, independent of recorded `cost_tokens` so it never double-counts. REST `GET /api/v1/usage` (`app/core/usage_router.py`, JWT-role-gated, org-scoped). **Producers wired:** webhook delivery (0.99 ◬ each, central in `deliver_event`) + CSV export + AI inference (`cube6_ai` pipeline, quantity = total provider calls) + API-key auth (0.01 ◬/authenticated call, central in `resolve_principal`, best-effort so metering never blocks auth) call `record_usage` (best-effort, org-scoped) so the stream fills. **Stripe charge wired:** `summarize_usage(session_id=…)` scopes the metered summary to one session; `cube8_tokens.payment_service.estimate_metered_cost` + `create_usage_billing_checkout` turn that ◬→USD estimate into a real pay-for-what-you-used Stripe Checkout (floored at $0.50; keys read from `settings`, never hardcoded), completed by the existing `handle_payment_completed` webhook. Metering → billing loop is closed end-to-end.

### Embed Roadmap (3 Modes — Planned)
| Mode | Description | Status |
|------|-------------|--------|
| **Full Embed** | iframe with postMessage API or Web Component (`<exel-polling>`) | **Consumer SDK shipped** (`public/embed/exel-polling.js`); app-side event emit deferred |
...
| 6 AI Pipeline | ~87% | **87** | 193 | CRS-11→14 | C6-3/C6-5 + B1 DONE — offline theming E2E on 25 synthetic + full **5,000 real** responses (`v04.1_5000.csv`), determinism proven on real data (identical inputs → identical themes). Remaining: B3 (scale verify) |
...
- **Language Lexicon:** 1,793 keys × 33 languages, `t()` fallback (translation → English → raw key). The **Innovation Pod** (`soi.pod.*`, /soi-session) is fully translated in all 33 languages and gated with zero placeholders (`tests/lexicon-coverage.test.mjs`, pod-scoped, in `test:ci`; the Sign/Create-Doc surface by `sign-i18n-all`). Every other app falls back to English until translated in **its own scope** — do not bulk-translate the whole lexicon (operator 2026-09-13: "only updating Innovation Pod"). Pod/sign fills live in the lazy per-language store `lib/i18n-sign/`. Per-cube key groups (Cubes 1-9 + SDK), admin approval gate (`explore@eXeL-AI.com`), `t()` fallback chain (translation → English → raw key). Files: `frontend/lib/lexicon-data.ts`, `frontend/lib/lexicon-context.tsx`, `frontend/lib/lexicon-translations.ts`, `frontend/components/language-lexicon.tsx`
