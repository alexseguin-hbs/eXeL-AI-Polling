...
- **Usage metering — IMPLEMENTED (foundation)** — append-only per-org `UsageRecord` (`app/models/usage_record.py`: org_id · metric · quantity · cost_tokens ◬ · api_key/session/scope provenance, indexed by org+metric+time). Service `app/core/usage_service.py` (`record_usage` with metric whitelist; `summarize_usage` per-metric + total over an optional window, stable shape). REST `GET /api/v1/usage` (`app/core/usage_router.py`, JWT-role-gated, org-scoped). Remaining: call `record_usage` from producers (API-key auth, AI inference, webhook delivery, export) so the stream fills, + Stripe billing rollup.
