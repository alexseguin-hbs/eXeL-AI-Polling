...
- **Usage metering — IMPLEMENTED (foundation)** — append-only per-org `UsageRecord` (`app/models/usage_record.py`: org_id · metric · quantity · cost_tokens ◬ · api_key/session/scope provenance, indexed by org+metric+time). Service `app/core/usage_service.py` (`record_usage` with metric whitelist; `summarize_usage` per-metric + total over an optional window, stable shape). REST `GET /api/v1/usage` (`app/core/usage_router.py`, JWT-role-gated, org-scoped). **Producers wired:** webhook delivery (0.99 ◬ each, central in `deliver_event`) + CSV export call `record_usage` (best-effort, org-scoped) so the stream fills. Remaining: meter AI inference + API-key auth calls, + Stripe billing rollup.
