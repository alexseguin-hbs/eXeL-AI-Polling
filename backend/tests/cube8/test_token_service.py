...
Tests:
  ...
  - 웃 rate table (hi_rates.py): resolve_hi_rate, get_all_rates
...
class TestGetUserTokenBalance:
    ...
    async def test_aggregates_balance(self):
        ...
        e1 = make_token_ledger(delta_si=1.0, delta_hi=0.0, delta_ai=5.0, lifecycle_state="pending")
        e2 = make_token_ledger(delta_si=3.0, delta_hi=0.5, delta_ai=15.0, lifecycle_state="approved")
        e3 = make_token_ledger(delta_si=2.0, delta_hi=0.0, delta_ai=10.0, lifecycle_state="finalized")
        ...
        assert result["total_si"] == 6.0
        assert result["total_hi"] == 0.5
        assert result["total_ai"] == 30.0
    ...
    async def test_zero_balance(self):
        ...
        assert result["total_si"] == 0
        assert result["total_hi"] == 0
...
class TestGetAllRates:
    ...
    def test_rate_structure(self):
        """Each rate entry should have country, state, hi_rate, currency."""
        from app.core.hi_rates import get_all_rates
        rates = get_all_rates()
        for rate in rates:
            assert "country" in rate
            assert "hi_rate" in rate
            assert "currency" in rate
            assert rate["currency"] == "USD"
            assert rate["hi_rate"] > 0
