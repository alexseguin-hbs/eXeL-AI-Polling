...
HI-token conversion (`dollars_to_hi_tokens`) → jurisdiction rate (`resolve_human_rate`)
...
from app.cubes.cube8_tokens.service import VALID_TRANSITIONS, dollars_to_hi_tokens
...
def run_harness_cube8() -> dict:
    ...
    fee_hi = dollars_to_hi_tokens(_FEE_USD)          # real HI-token conversion
    ...
    sig_payload = json.dumps(
        {k: result[k] for k in ("tokens", "fee_hi_tokens", "lifecycle_path")},
        sort_keys=True,
    )
...
def render(r: dict) -> str:
    ...
    out.append(f"  fee:           ${r['fee_usd']} → {r['fee_hi_tokens']} 웃 (HI conversion)")
