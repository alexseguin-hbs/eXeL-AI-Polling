...
async def pod_synthesis(payload: PodSynthesisRequest) -> PodSynthesisResponse:
    ...
    context = (
        ...
        f"PERSON-HOURS COUNTED (each claim capped to the clock, summed): {f.witnessed_hours}\n"
        f"POD CLOCK, UNDER ITS OWN NAME: {f.witnessed_for or '—'} in {len(f.segments)} segment(s)"
        + (": " + "; ".join(f"{g.get('hhmmss', '?')}" for g in f.segments) if f.segments else "") + "\n"
        f"OUTCOMES, ONE PER MEMBER: {' | '.join(o for o in f.member_outcomes if o) or '—'}\n"
        f"웃 SETTLED (웃 = M × T, M={f.m} accepted by the trio before Start): {f.yug_yok}\n"
