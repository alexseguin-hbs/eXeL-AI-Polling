...
# ═══════════════════════════════════════════════════════════════════
# CQS SCORING — Content Quality Score (post-ranking)
# ═══════════════════════════════════════════════════════════════════
...
async def score_cqs(
    ...
    # Load session for provider + weights
    result = await db.execute(select(Session).where(Session.id == session_id))
    session = result.scalar_one_or_none()
    if session is None:
        raise ValueError(f"Session {session_id} not found")
    ...
    # Fetch responses with theme assignments in the top cluster
    ...
    # Filter to top theme cluster with >95% confidence
    eligible = []
