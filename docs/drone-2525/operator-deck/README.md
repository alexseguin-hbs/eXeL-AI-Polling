...
**HEAD is `drone-2525_r.153.html` — SERVED AS THE PLAYABLE at `/drone-2525/play.html`.** r.153 (Claude Code, 2026-09-26, sha256 3c3de52165ea9138…, `HASHES_r153.sha256`): **the browser tab reads "eXeL Drone-2525"** — the `<title>` was the last place the stale early deck name "eXeL ECO-2525 · MoT · SSSES" survived, so the phone tab said ECO; now exactly "eXeL Drone-2525" (title-only, byte-gated; operator ask `docs/asks/2026-09-26_drone_tab_name.md`, `CLAUDE_CODE_NOTES_r153.md`). Previous HEAD: r.152 (Claude Code, 2026-09-24, sha256 e15cd3993aa378c95…, `HASHES_r152.sha256`): **the full-screen control is an icon, not a word** — the top-bar (`btnFullBar`) and magazine-bar (`btnFull`) FULL/EXIT word buttons become the Mission-Planning maximize (`⤢`) / minimize (`⤡`) icons (the SoI-2525 ChartFrame and Security-2525 Mission Planning convention); the top-bar maximize icon sits at the upper-right at a comfortable tap size; the fullscreen behaviour (requestFullscreen/exitFullscreen, `#app.full` hiding the words) is unchanged; the pop-ups reconfirmed on the served deck (all up in Target-Up, amber on TARGET, red on APPROVE, a marked+approved pop fires — no fix needed). Operator ask `docs/asks/2026-09-24_drone_popups_box_maximize.md` (`CLAUDE_CODE_NOTES_r152.md`). Previous HEAD: r.151 (Claude Code, 2026-09-24, sha256 3c4d297eec612261a…, `HASHES_r151.sha256`): **pop-up targets fire again** — a lawn pop the LOCK reaches (`LOCK_REACH_PX`, 48 px) is now fireable; r.150's non-plate FIRE gate had used 32 px against the pop's base, 1.2 m below the drawn silhouette, so a marked, approved TURRET / CAPITAL pop was refused `TARGET_OFF_PICTURE` ("pop-up targets no longer work"). One value: the non-plate FIRE tolerance is `LOCK_REACH_PX`, so the gate accepts exactly what the LOCK accepts; `pipOn`, the lock, the draw and plate handling are untouched. Item 5 of the 48-agent fleet's order on r.147 (`CLAUDE_CODE_NOTES_r151.md`). r.150 (Claude Code, 2026-09-24, sha256 e8d553ed035352b1c…, `HASHES_r150.sha256`): **the fire gate reads the picture** — a round or a tag is spent only on a red box the bullseye is on; FIRE says where the box is and costs nothing; no tag without a red box; no approval on a dead plate; the AI re-arms on reload and reads the HIT for 1.4 s; the deck's own SSSES row is gone. Items 1–4 of the 48-agent fleet's order on r.147 (`CLAUDE_CODE_NOTES_r150.md`). r.149 (Claude Code, 2026-09-23, sha256 20499eed97482ba22…, `HASHES_r149.sha256`): **a box only around the marked target** — no edge markers unless targeted; amber on TARGET, red on APPROVE, the voxel in the 3D picture for every marked slot; the three range modes re-stated. r.148 (Claude Code, 2026-09-23, sha256 213a56fc08d41f510…, `HASHES_r148.sha256`): **the eye outranks the mark** — the 48-agent fleet's fold on r.147: a mark is held only while the bullseye is on it, one door for TARGET / keys / voice that keeps a mark's phase and author, every key obeys the reach, the AI spots through the bullseye and never moves the seated head, a FIRE with no box names NONE, the board under the strip, the wire's APPROVE author from the envelope and a peer HIT needing red on record (r.147's rule had run only on an empty board — corrected). r.147 (Claude Code, 2026-09-23, sha256 2476f68a8a8835e14…, `HASHES_r147.sha256`): **LOCK, T1 · T2 · T3 and key 1 are the targets nearest the bullseye on the picture** — never the nearest in metres, never off the picture (TARGET / APPROVE / FIRE had kept resetting to the 50 m left target); one rule for the range, the Capital and every kind. r.146 (Claude Code, 2026-09-23, sha256 b3b17cbde2f49a145…, `HASHES_r146.sha256`): **deferred QA rows decide on evidence, not the runner's clock** — `DRAW_COMPLETES` and `ASM_MARKED_BY_THE_LOOP` poll for the frame / the loop's mark up to a declared ceiling (a cold CI runner had been judged "no frame completed" at 1.5 s for ever, Deploy #950); proven under a 30× CPU throttle. r.145 (Claude Code, 2026-09-23, sha256 3a2d43e22387a2183…, `HASHES_r145.sha256`): **lane markers at 100, 200 and 300 m** — numbered boards on posts at the lane edges like the range photograph, for your lane and the lanes beside it. r.144 (Claude Code, 2026-09-23, sha256 2ffd1288883d2e1d3…, `HASHES_r144.sha256`): r.144 (Claude Code, 2026-09-23, sha256 2ffd1288883d2e1d3…, `HASHES_r144.sha256`): **the targets on each of the 42 lanes** — every lane carries the same eleven silhouettes (462 plates, gated), and the neighbouring lanes now draw all of theirs, dim, while marking and firing stay on your lane. r.143 (Claude Code, 2026-09-23, sha256 c4c187b415aebed14…, `HASHES_r143.sha256`): r.143 (Claude Code, 2026-09-23, sha256 c4c187b415aebed14…, `HASHES_r143.sha256`): **no horizon line; a white bullseye that turns red near a target; the bracket red with it; LOCK is the thing in the crosshair** (with every target up it had named the nearer plate beside the pip). r.142 (Claude Code, 2026-09-23, sha256 4c7d6ee31fa605735…, `HASHES_r142.sha256`): r.142 (Claude Code, 2026-09-23, sha256 4c7d6ee31fa605735…, `HASHES_r142.sha256`): **reload by hand on QUAL, the same order as the actual test** — the program names its silhouettes (left / centre / right) and is the same on every lane, every time (the per-lane seed retired, operator decision); ten targets a phase; the tower's rest says PRESS RELOAD and loads nothing; an empty magazine refuses FIRE; four magazines as issued. r.141 (Claude Code, 2026-09-23, sha256 d177187681613f8d6…, `HASHES_r141.sha256`): r.141 (Claude Code, 2026-09-23, sha256 d177187681613f8d6…, `HASHES_r141.sha256`): **life-size at 12 in** — one focal length at 1× (12 in × 152.3 px/in, iPhone 12 Pro Max), the typed 38° half-angle gone (r.130–r.140 corrected: the field was 76°); a 50 m F is 24 px wide at 1×, a 300 m E 6 px; the 50 L / 50 R stand at ±4.6 m, the edges of a portrait phone. r.140 (Claude Code, 2026-09-23, sha256 0f44f0d8bfd83c0e5…, `HASHES_r140.sha256`): r.140 (Claude Code, 2026-09-23, sha256 0f44f0d8bfd83c0e5…, `HASHES_r140.sha256`): **the dark green plates and the 25 m Alt-C sheet** — the F-type is 26 × 21 in (r.138's 19.5 × 20 corrected), E 19.5 × 40 in; a TARGETS picker adds the 17 × 22 in Alt-C sheet at 25 m with the ten silhouettes scaled by 25/range (measured: the same height on screen as the real target at its range); paper never falls — a hit is a hole, DOWN scores a silhouette out, QUAL · 40 runs on the sheet. r.139 (Claude Code, 2026-09-23, sha256 e8b52069c3b2ec8e9…, `HASHES_r139.sha256`): r.139 (Claude Code, 2026-09-23, sha256 e8b52069c3b2ec8e9…, `HASHES_r139.sha256`): **landscape and portrait, like Mission Planning** — a phone is a phone in both orientations (device class by the short side); FULL fills the whole screen in every orientation (r.138 left the picture at 36 % height — corrected); FULL beats the desk layout; portrait stacks the magazine line and the strip, landscape sets them side by side; rotation re-lays out; the deck-QA gate runs both orientations in CI. r.138 (Claude Code, 2026-09-23, sha256 c8449d701799ff23a…, `HASHES_r138.sha256`): r.138 (Claude Code, 2026-09-23, sha256 c8449d701799ff23a…, `HASHES_r138.sha256`): **true-scale silhouettes, justified labels, the zoom law, magazine + reload, full screen** — F 0.495 × 0.508 m and E 0.495 × 1.016 m (a 300 m E is 1.6 px at 1×, the 3 mrad floor keeps it hittable); left targets' labels right-justified, right targets' left-justified, centre above, same-side labels stacked; the turret sees 3× in QUAL · 40 and 30× in training, every craft 30×; four 10-round magazines in QUAL (the phase rest reloads on the record), 30-round magazines elsewhere, RELOAD + round counter + zoom at the top of the picture, an empty magazine refuses FIRE; FULL hides the bars, strip and dock and keeps the sticks, TARGET · APPROVE · FIRE and RELOAD. r.137 (Claude Code, 2026-09-23, sha256 8e3ef876699b5a0b9…, `HASHES_r137.sha256`): r.137 (Claude Code, 2026-09-23, sha256 8e3ef876699b5a0b9…, `HASHES_r137.sha256`): **the range the operator described** — TRAINING · RESET / DOWN stand every target (a hit one comes back after 3 s, or stays down); QUAL · 40 is the Army IWQ Table VI 40-round day fire (18 engagements in 4 phases, singles to quads up TOGETHER for 5/8/12/16 s, 3 s between engagements, 9 s between phases, one round per silhouette, unengaged = unfired miss, the 50 R first; engagements 15–18 DECLARED); the 50 m targets are left and right (11 silhouettes). r.136 (Claude Code, 2026-09-19, sha256 2bcd7a574e5044f72…, `HASHES_r136.sha256`): r.136 (Claude Code, 2026-09-19, sha256 2bcd7a574e5044f72…, `HASHES_r136.sha256`): **the fold of the 48-agent fleet's synthesis** — the r.135 regression closed (an AI mark in a room is fired by the seat that owns the AI; the approver is told the other seat fires), every peer release reaches the shooter, no red box crosses into LIVE, two QA rows assert what they name. r.135 (Claude Code, 2026-09-19, sha256 882287ecc3e2abc6f…, `HASHES_r135.sha256`): **the fold of the fleet's second (adversarial) reviewers** — three r.134 regressions closed (the uncalled frame tail, a lapse row from both phones, a DROPPED link undone by the next message); the fire gate's remaining members (the round must be open, the approver's seat never fires the marker's box, a tag needs a red box, an expired target scores nothing, no AI passport from a row, CH5 re-approves after a miss); the record carries the decision's content and the actor; no session id or plate id reaches the player. r.134 (Claude Code, 2026-09-19, sha256 840cf55c6c0181503…, `HASHES_r134.sha256`): **the fold of team-test wave 3 and the 48-lens fleet review** — authority has one consumer (a scene change, a reset, a lost approver or a dead target releases the red box with a row), RESET and mode changes are on the record and travel, the link has a heartbeat, VOICE reports the outcome, MAP keeps the strip, the AI member is an actor (`ASM@<device>`) that never marks a dead plate, refusals draw above the intro, the host shows the other team's six digits, the two-phone gate runs in Deploy and reads the approver's picture, one HEAD across register/README/JSON/ledger is gated. r.133 (Claude Code, 2026-09-19, sha256 d10ae69bc53d1a73…, `HASHES_r133.sha256`): **the 38-AsM team-test fold** — the AI member
